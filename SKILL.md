...
license: GPL-2.0+
compatibility: Requires PHP 7.4+, WordPress 6.0+ environment
metadata:
  author: Terra Coder
  version: 1.0.0
...
- Use a unique prefix (2–3 letters) for all functions, classes, hooks, and text domains to avoid collisions.
  ...
  function map_sanitize_input( $value ) { ... }
...
function map_admin_menu() {
    ...
    add_menu_page(
        ...
        'my-plugin',                           // Menu slug
        'map_render_admin_page',              // Callback
...
function map_register_settings() {
    register_setting( 'map_settings_group', 'map_option_name', array(
        'type'              => 'string',
...
function map_my_shortcode( $atts ) {
