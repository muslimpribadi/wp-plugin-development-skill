...
| `$args` | array | No | Endpoint configuration (methods, callbacks, args, schema) |
...
function myplugin_hello_handler() {
...
function myshop_get_product( $request ) {
...
function myshop_register_routes() {
    register_rest_route( 'my-shop/v1', '/products', array(
        array(
            'methods'             => WP_REST_Server::READABLE,
            'callback'            => 'myshop_get_products',
